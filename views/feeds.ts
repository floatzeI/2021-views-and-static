...
const GroupFeedComponent = (props: {
	feedId: number;
	status: string | null;
	group: {
		name: string;
	...
	const url = `/groups/${props.group.id}/${convertToSeoName(props.group.name)}`;
	return `            <li class="list-item">
    
    <a href="${url}" class="list-header" ><img  class='header-thumb' src='${props.group.image}'/></a>
                <div class="list-body">
                    <p class="list-content"><a href='${url}'>${xss(props.group.name)}</a><p class='feedtext linkify'>"${xss(props.status || '')}"</p> (posted by <a href='/users/${props.user.id}/profile' class='text-name'>${xss(props.user.name)}</a>)</p>
                            <span class="xsmall text-date-hint">${moment(props.createdAt).format(dateFormat)}</span>
    
                        <a href="https://www.roblox.com/abusereport/Feed?id=${props.feedId}&amp;redirectUrl=https%3A%2F%2Fwww.roblox.com%2Fhome" class="abuse-report-modal">
...
const UserFeedComponent = (props: {
    ...
    <a href="https://www.roblox.com/users/${props.id
		}/profile" class="avatar avatar-headshot-md list-header">
        <img src="${xss(props.thumbnail)}" class="avatar-card-image">
    </a>

    <div class="list-body">
        <p class="list-content"><a href='https://www.roblox.com/users/${props.id
		}/profile' class='text-name'>${xss(
			props.name
		)}</a><p class='feedtext linkify'>&quot;${xss(
			props.status || ''
		)}&quot;</p></p>
                <span class="xsmall text-date-hint">${moment(
			props.createdAt
		).format(dateFormat)}</span>

            <a href="https://www.roblox.com/abusereport/Feed?id=${props.feedId
		}&amp;redirectUrl=https%3A%2F%2Fwww.roblox.com%2Fhome" class="abuse-report-modal">
                <span class="icon-report"></span>
            </a>
    </div>
</li>`;
...
export default (
	user: IUserInfo,
	feedArray: Array<IUserFeed | IGroupFeed>,
	myStatus: string,
	htmlOnly?: boolean
): string => {
	if (htmlOnly) {
		return `<ul class="vlist feeds">
                    ${feedArray
				.map((v) => {
					if (v.type === 'User') {
						return UserFeedComponent(v);
					} else if (v.type === 'Group') {
						return GroupFeedComponent(v);
					} else {
						return '';
					}
				})
    ...
    <div class="section in-app-feeds feeds-container" id="feed-container"
         data-update-status-url="/home/updatestatus">
        <h1 class="feeds-title">My Feed</h1>
        <div class="section-content">
            <div class="form-horizontal" id="statusForm" role="form">
                <div class="form-group">
                    <input class="form-control input-field" id="txtStatusMessage" maxlength="254" placeholder="What are you up to?" value="${xss(
		myStatus
	)}"/>
                    <p class="form-control-label">Status update failed.</p>
                    <span class="icon-checkmark-on success-icon"></span>
                </div>
                <a  type="button" class="btn-primary-md btn-fixed-width" id="shareButton">Share</a>
                <span id="loadingImage" class="spinner spinner-default share-login"></span>
            </div>
    ...
    <script onerror='Roblox.BundleDetector && Roblox.BundleDetector.reportBundleError(this)' data-monitor='true' data-bundlename='DynamicLocalizationResourceScript_Common.AlertsAndOptions' type='text/javascript' src='/rbxcdn/js/1e148db91b54661ad571d33042e374da2c6d63e6bdeb458c770c6cd5b8442ffb.js'></script>
        ...
        <script>
            Roblox.XsrfToken.setToken('9ZhE+NAC8TcZ');
        </script>
    ...
    <script type="text/javascript">
        ...
        <script onerror='Roblox.BundleDetector && Roblox.BundleDetector.reportBundleError(this)' data-monitor='true' data-bundlename='page' type='text/javascript' src='/rbxcdn/js/f034800f0b491fa2008d3c4668c216bd.js'></script>
