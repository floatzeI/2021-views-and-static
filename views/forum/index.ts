...
export default (session: IUserInfo, groups: any) => {
...
<div id="forum-nav" style="text-align: right">
	...
	<a id="ctl00_cphRoblox_NavigationMenu2_ctl00_SearchMenu" class="menuTextLink" href="/Forum/Search/default.aspx">Search</a>
				...
				<table width="100%" cellspacing="2" cellpadding="0">
					<tbody><tr>
						<td align="left">
							<span class="normalTextSmallBold">Current time: </span><span class="normalTextSmall">${moment().format(
		'MMM D, h:mm A'
	)}</span>
						</td>
						<td align="right">
...
<span>
    <form method="GET" action="/Forum/Search.aspx">
        ...
        <input type="submit" value="Go" class="translate btn-control btn-control-medium forum-btn-control-medium">
                ...
                ${groups
			.map((v: any) => {
				let html = `<tr class="table-header forum-table-header">
    
                    <th class="first" colspan="2"><a id="ctl00_cphRoblox_ForumGroupRepeater1_ctl01_GroupTitle" class="forumTitle" href="/Forum/ShowForumGroup.aspx?ForumGroupID=${v.id
					}">${xss(
						v.name
					)}</a></th><th style="width:50px;white-space:nowrap;">&nbsp;&nbsp;Threads&nbsp;&nbsp;</th><th style="width:50px;white-space:nowrap;">&nbsp;&nbsp;Posts&nbsp;&nbsp;</th><th style="width:135px;white-space:nowrap;">&nbsp;Last Post&nbsp;</th>
    
                    </tr>`;
				let subs = v.subCategories;
				for (const item of subs) {
					html += `
                        <tr class="forum-table-row" style="border-bottom: 1px solid #c3c3c3;">
                            <td colspan="2" style="width:80%;"><a class="forum-summary" href="/Forum/ShowForum.aspx?ForumID=${item.id
						}">
                            ...
                            <td class="forum-centered-cell" align="center">
                                <span class="normalTextSmaller">${item.threads.toLocaleString()}</span>
                            </td>
                            <td class="forum-centered-cell" align="center">
                                <span class="normalTextSmaller">${item.posts.toLocaleString()}</span>
                            </td>
                            <td align="center">
                            ${item.latestPost
							? `
                                <a class="last-post" href="/Forum/ShowPost.aspx?PostID=${item.latestPost.threadId
								? item.latestPost.threadId +
								'#' +
								...
								item.latestPost.createdAt
                                    ...
                                    <div>
                                        ${xss(item.latestPost.username)}
                                    </div>
                                </span>
                                <span class="normalTextSmaller notranslate"><div class="notranslate">${xss(
								item.username
							)}</div></span></a>`
							: ''
						}
