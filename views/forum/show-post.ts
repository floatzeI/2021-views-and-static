...
export default (
	...
	subCategory: any,
	nextThreadId: any,
	previousThreadId: any
) => {
	let pageButtons = [];
	// add 1 to account for thread post itself
	let maxPages = Math.floor(totalPosts / 25) + 1;
	for (let i = page - 2; i < maxPages; i++) {
		...
		if (i === page) {
			pageButtons.push(
				`<span class="normalTextSmallBold">[${i.toLocaleString()}]</span>`
			);
		} else {
			pageButtons.push(
				`<span class="normalTextSmallBold">${i.toLocaleString()}</span>`
	...
	const nextPrevious = `<td class="tableHeaderText table-header-right-align" align="right">
    <a id="ctl00_cphRoblox_PostView1_ctl00_PostList_ctl00_PreviousThread" class="linkSmallBold" href="${previousThreadId
			? `/Forum/ShowPost.aspx?PostID=${previousThreadId}`
			: ''
		}">Previous Thread</a>&nbsp;
    <span class="normalTextSmallBold">::</span>&nbsp;
    <a id="ctl00_cphRoblox_PostView1_ctl00_PostList_ctl00_NextThread" class="linkSmallBold" href="${nextThreadId ? `/Forum/ShowPost.aspx?PostID=${nextThreadId}` : '#'
		}">Next Thread</a>&nbsp;
</td>`;
    ...
    <link onerror='Roblox.BundleDetector && Roblox.BundleDetector.reportBundleError(this)' rel='stylesheet'  href='/rbxcdn/static/css/page___54a90a103086d5ffeb5549d4d9d1c1c5_m.css/fetch' />
...
<nobr>
    ...
    <a id="ctl00_cphRoblox_ThreadView1_ctl00_Whereami1_ctl00_LinkForumGroup" class="linkMenuSink notranslate" href="/Forum/ShowForumGroup.aspx?ForumGroupID=${category.id
		}">${xss(category.name)}</a>
</nobr>
<nobr>
    <span id="ctl00_cphRoblox_ThreadView1_ctl00_Whereami1_ctl00_ForumSeparator" class="normalTextSmallBold"> » </span>
    <a id="ctl00_cphRoblox_ThreadView1_ctl00_Whereami1_ctl00_LinkForum" class="linkMenuSink notranslate" href="/Forum/ShowForum.aspx?ForumID=${subCategory.id
		}">${xss(subCategory.name)}</a>
</nobr>
</div>
</span>
</td>
...
<div id="forum-nav" style="text-align: right">
	...
	<tr>
        <td colspan="2" align="left">
            <h2 id="ctl00_cphRoblox_PostView1_ctl00_PostTitle" cssclass="notranslate" style="margin-bottom:20px;font-size:15px;">${xss(
			postData.title
		)}</h2>
        </td>
    </tr>
    <tr>
    <td colspan="2">
        <table id="ctl00_cphRoblox_PostView1_ctl00_PostList" class="tableBorder" style="width:100%;" cellspacing="1" cellpadding="0" border="0">
	<tbody><tr>
		<td class="forumHeaderBackgroundAlternate" colspan="2" style="height:20px;"><table class="forum-table-header" style="width:100%;border-collapse:collapse;" cellspacing="0" cellpadding="0" border="0">
			<tbody><tr>
    ...
    ${posts
			.map(
				(v: {
					...
					const isOnline = moment(v.online).isSameOrAfter(
						moment().subtract(5, 'minutes')
					);
					return `<tr class="forum-post" data-post-id="${v.postId}">
        <td class="forum-content-background" style="width:150px;white-space:nowrap;" valign="top">
        <table border="0">
			<tbody><tr>
                <td>
                    <img src="${isOnline
							? '/img/user_IsOnline.gif'
							: '/img/user_IsOffline.gif'
						}" alt="${xss(v.username)} is ${isOnline ? 'Online' : 'Offline'
						}." style="border-width:0px;">&nbsp;<a class="normalTextSmallBold notranslate" href="/users/${v.userId
						}/profile">${xss(v.username)}</a><br></td>
			</tr><tr>
                <td>
                    <a href="/users/${v.userId
						}/profile" style="width:100px;height:100px;position:relative;">
                        <img src="${v.thumbnailUrl
						}" style="border-width:0px;width:100px;height:100px;">
                        ${
							/*<img src="https://web.archive.org/web/20170727093112im_/https://www.roblox.com/Thumbs/BCOverlay.ashx?username=iDanstar" style="border-width:0px;position:absolute;left:0px;bottom:0px;">*/ ''
						}
                    </a>
                </td>
			</tr><tr>
				<td><span class="normalTextSmaller"><b>Joined: </b>${moment(
							v.userJoinDate
						).format('DD MMM YYYY')}</span></td>
			</tr><tr>
				<td><span class="normalTextSmaller"><b>Total Posts: </b>${v.postCount
						}</span></td>
			</tr><tr>
                <td style="height:20px;">
                    <span class="normalTextSmaller primaryGroupInfo notranslate" username="${xss(
							v.username
        ...
        <td class="forum-content-background" valign="top">
            <table style="width:100%;border-collapse:collapse;table-layout:fixed;overflow:hidden;word-wrap:break-word;" cellspacing="0" cellpadding="3" border="0">
            <tbody>
            <tr>
				<td colspan="2"><span class="normalTextSmaller">${moment(v.createdAt).format(
							'DD MMM YYYY hh:mm A'
						)}<a name="${v.userId}"></a></span></td>
            </tr>
            <tr>
				<td colspan="2" style="height:125px;" valign="top"><span class="normalTextSmall notranslate linkify post-text" data-post-id="${v.postId}">${xss(
							v.post
						).replace(/\n/g, '<br />')}</span></td>
            </tr>
            <tr>
					...
					<span class="post-response-options">
						<span class="ReportAbuse">
							<span class="AbuseButton">
								${session.isModerator ? `
								<a href="#" data-post-id="${v.postId}" class="delete-post-moderator">Remove Post</a>
								` : `
								<a href="/AbuseReport/ForumPost.aspx?PostID=${v.postId
						}&amp;RedirectUrl=%2fForum%2fShowPost.aspx%3fPostID%3d${v.postId
						}">Report Abuse</a>`}
							</span>
						</span>
					</span>
...
<tr>
		<td colspan="2" style="height:20px;"><table class="forum-table-header" style="width:100%;border-collapse:collapse;" cellspacing="0" cellpadding="0" border="0">
			<tbody><tr>
				<td style="width:50%;" align="left"></td>${nextPrevious}
			</tr>
		</tbody></table></td>
	</tr>
</tbody></table>
        <span id="ctl00_cphRoblox_PostView1_ctl00_Pager"><table style="width:100%;border-collapse:collapse;" cellspacing="0" cellpadding="0" border="0">
    <tbody>
    <tr>
        <td>
            <span class="normalTextSmallBold">Page ${page} of ${maxPages}</span>
        </td>
        <td align="right">
        ${maxPages > 1
			? `
            <span>
                <span class="normalTextSmallBold">Go to page: </span>
                ${page > 1
				? `<span class="normalTextSmallBold">&nbsp;</span><a id="ctl00_cphRoblox_PostView1_ctl00_Pager_Previous" class="normalTextSmallBold" href="/Forum/ShowPost.aspx?PostID=${postData.id
                ...
                ${maxPages > page
				? `<span class="normalTextSmallBold">&nbsp;</span><a id="ctl00_cphRoblox_PostView1_ctl00_Pager_Next" class="normalTextSmallBold" href="/Forum/ShowPost.aspx?PostID=${postData.id
				}&Page=${page + 1}">Next</a>
                </span>`
				: ''
			}
    ...
    <tr>
        <td valign="bottom" align="middle">
            <a id="ctl00_cphRoblox_ThreadView1_ctl00_NewReplyLinkBottom" class="btn-control btn-control-medium verified-email-act" href="/Forum/AddReply.aspx?PostID=${postData.id
		}">
                Reply
            </a>
        </td>
...
${session.isModerator ? `<script>
$(document).ready(function() {
	$(document).on('click', '.delete-post-moderator', function(e) {
		e.preventDefault();
		var postId = $(this).attr('data-post-id');
		$('.post-text[data-post-id="'+postId+'"]').text("[ Content Deleted ]");
		console.log('[info] delete post', postId);
		
		$.ajax({
                    type: "POST",
                    url: "/admin-api/api/delete-forum-post",
                    data: {
                        postId: postId,
                    },
                    complete: function(xhr, status) {
                        if (xhr.status == 200 ) {
							...
							alert("Could not delete post #" + postId + ": "+(xhr.responseText));
						}
                    },
                    dataType: "json"
		});
