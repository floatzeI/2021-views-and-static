				...
				<table width="100%" cellspacing="2" cellpadding="0">
					<tbody><tr>
						<td valign="bottom" align="left">
                            <a id="ctl00_cphRoblox_ThreadView1_ctl00_NewThreadLinkTop" class="btn-control btn-control-medium verified-email-act" href="/Forum/AddPost.aspx?ForumID=${subCategory.id}">
                                New Thread
                            </a>
						</td>
						<td align="right">
...
<span>
    <form method="GET" action="/Forum/Search.aspx">
        <input type="hidden" name="ForumID" value="${subCategory.id}">
        <span class="normalTextSmallBold">Search This Forum:</span>
        <input name="query" type="text" maxlength="50" class="notranslate" size="20">
        <input type="submit" value="Go" class="translate btn-control btn-control-medium forum-btn-control-medium">
                ...
                <table class="table" width="100%" cellspacing="1" cellpadding="2" border="0">
                <tbody>

                <tr class="table-header forum-table-header">
    
                    <th class="first" colspan="2"><a id="ctl00_cphRoblox_ForumGroupRepeater1_ctl01_GroupTitle" class="forumTitle">Subject</a></th>
                    <th style="width:50px;white-space:nowrap;">&nbsp;&nbsp;Author&nbsp;&nbsp;</th>
                    <th style="width:50px;white-space:nowrap;">&nbsp;&nbsp;Replies&nbsp;&nbsp;</th>
                    <th style="width:50px;white-space:nowrap;">&nbsp;&nbsp;Views&nbsp;&nbsp;</th>
			...
			.map((v) => {
				return `
                        <tr class="forum-table-row">
                            <td style="width:25px;" valign="middle" align="center">
                            ${v.read
						? `<img title="Post (Read)" src="/img/thread-read.png" style="border-width:0px;">`
						: v.views >= 500
							? `<img title="Popular Post (Not Read)" src="/img/thread-popular.png" style="border-width:0px;">`
							: `
                                <img title="Post (Not Read)" src="/img/thread-unread.png" style="border-width:0px;">`
					}
                            </td>
                            <td style="width:80%;">
                                <a class="forum-summary" href="/Forum/ShowPost.aspx?PostID=${v.thread_id +
					(v.id !== v.thread_id ? '#' + v.id : '')
					}">
                                <div class="notranslate">
									${xss(v.title)}${session.isModerator ? ` <span class="delete-post-moderator" style="color: red;float: right;" data-post-id="${v.thread_id || v.id}">Delete</span>` : ''}
                                </div>
                                </a>
                            </td>
                            <td class="forum-centered-cell" align="center">
                                <a class="post-list-author notranslate" href="/users/${v.thread_author_id
					}/profile">
                                    <div class="thread-link-outer-wrapper">
                                        <div class="normalTextSmaller thread-link-container" style="font-size: 13px;">
                                            ${xss(v.thread_author_name)}
                                        </div>
                                    </div>
                                </a>
                            </td>
                            <td class="forum-centered-cell" align="center">
                                <span class="normalTextSmaller">${v.replies.toLocaleString()}</span>
                            </td>
                            <td class="forum-centered-cell" align="center">
                                <span class="normalTextSmaller">${v.views.toLocaleString()}</span>
                            </td>
                            <td style="width:100px;white-space:nowrap;" align="center">
                                <a class="last-post" href="/Forum/ShowPost.aspx?PostID=${v.thread_id || v.id
					}#${v.id}">
                                    <div>
			                            <span class="normalTextSmaller"><b>${v.is_pinned
						? 'Pinned Post'
						: moment(v.created_at).format(
							'hh:mm A'
                                    ...
                                    <div class="normalTextSmaller notranslate" style="font-size: 13px;">${xss(
						v.username
					)}</div>
                                </a>
                            </td>
